...
class Agent:
    def __init__(self, env):  # 需要输入游戏环境
        ...
        def create_model(input_state_shape):
            ...
            # 首先计算一个state value的平均值
            state_value = tl.layers.Dense(n_units=1)(layer_2)  # 这个是所有动作的平均值
            # 定义输出层的维度，与动作空间的维度一致 一维的列向量 （0，2）
            q_value = tl.layers.Dense(n_units=self.action_dim)(layer_2)  # 这个是对所有的动作价值打分
            mean = tl.layers.Lambda(lambda x: tf.reduce_mean(x, axis=1, keepdims=True))(q_value)  # 计算Q价值的平均值，保持维度
            advantage = tl.layers.ElementwiseLambda(lambda x, y: x - y)([q_value, mean])  # 用动作的价值减去平均值
            output_layer = tl.layers.ElementwiseLambda(lambda x, y: x + y)([state_value, advantage]) # 好的动作分数高，坏动作分数低
