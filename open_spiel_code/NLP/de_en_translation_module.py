...
de_tokenizer = get_tokenizer('spacy', language='de_core_news_sm')  # 这个是分词器模型，
...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.input_dim = input_dim  # 德语词汇的数量
        self.emb_dim = emb_dim  # 转换为vector的维度
        self.enc_hid_dim = enc_hid_dim  # encoder的节点数目
        self.dec_hid_dim = dec_hid_dim  # decoder的节点数目
        self.dropout = dropout  # inactive的隐藏层节点数目
        ...
        self.fc = nn.Linear(enc_hid_dim * 2, dec_hid_dim)  # 从encoder到decoder的线性转换
    ...
    def forward(self, src: Tensor) -> Tuple[Tensor]:
        ...
        hidden = torch.tanh(self.fc(torch.cat((hidden[-2,:,:], hidden[-1,:,:]), dim = 1)))  # 转换为encoder的隐藏层节点数
...
class Attention(nn.Module):
    def __init__(self,
        ...
        self.attn_in = (enc_hid_dim * 2) + dec_hid_dim  # attention的节点数
    ...
    def forward(self,
        ...
        src_len = encoder_outputs.shape[0]  # 德语的语言长度

        repeated_decoder_hidden = decoder_hidden.unsqueeze(1).repeat(1, src_len, 1)  # 复制英语的节点数

        encoder_outputs = encoder_outputs.permute(1, 0, 2) # 转换decoder的想要的tensor
