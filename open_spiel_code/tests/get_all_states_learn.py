...
# 这里的代码其实就是如何获得整个的游戏状态树，其实就是一个episode，返回值是一个all states
"""
...
def _get_subgames_states(state, all_states, depth_limit, depth,
    ...
    if state.is_terminal():  # 首先判断游戏是否结束
        if include_terminals:
            # Include if not already present and then terminate recursion.
            state_str = to_string(state)  # 把state转化为字符串类型
            if state_str not in all_states:
                all_states[state_str] = state.clone()  # 将state拷贝一份
        # 返回None
        return

    if depth > depth_limit >= 0:  # 如果深度超过了限制，返回None
        return
    is_mean_field = state.current_player() == pyspiel.PlayerId.MEAN_FIELD
    if (state.is_chance_node() and include_chance_states) or (is_mean_field
                                                              and include_mean_field_states) or not (
