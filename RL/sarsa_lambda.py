...
EPSILON = 0.9  # 又10%的概率随机选择动作，有90%的概率选择收益最大的动作
learning_rate = 0.1  # 学习率 要是加的多了 超过后面的值就不合适了
GAMMA = 0.9  # 折扣奖励
...
def rl():
    ...
    for episode in range(MAX_EPISODES):  # 一次又一次的玩游戏
        ...
        while not is_terminated:  # 如果游戏还没结束
            ...
            q_table += learning_rate * (q_target - q_predict)*eligibility_trace  # 更新Q表
            S = S_
            A = A_
            eligibility_trace *= GAMMA
