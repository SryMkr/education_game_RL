...
class GameLevel(object):
    ...
    def __init__(self, word_maker):
        ...
        self.cumulative_time = True  # 用来累加时间的变量
        self.countdown = self.word_maker.saved_time  # 初始化倒计时为之前省下的时间
        ...
        self.click_pronunciation = 0  # 玩家采用点击的方式听听力的次数
        self.press_Enter = 0  # 玩家被动听发音的次数
        self.click_Enter = 0  # 玩家采用点击的方式检查拼写的次数
    ...
    def split_Word(self, word):  # 首先输入一个单词
        ...
        self.contact_rect_list = [i for i in range(len(letter_list))]  # 存储框里一共可以放的坐标
        self.unreserved = [i for i in range(len(letter_list))]  # 存储框里还可以放的坐标
    ...
    def letter_move(self):
        ...
        if not self.letter_contact:  # 如果当前没有选中单词并且还有位置可以选择
            if len(self.occupied_rect) != len(self.current_word):
                # 首先要判定鼠标点击了哪个字母 字典{字母_位置：字母的Rect}
                for letter, Rect in self.letter_Rect.items():
                    # 鼠标点击事件是点了以后一直响应
                    if Rect.collidepoint(self.word_maker.mouse_current_x, self.word_maker.mouse_current_y) \
                            and self.word_maker.click_event:
                        game_Sound('game_sound/mouse_click_1.mp3', 0.2)  # 游戏鼠标的声音
                        self.letter_contact = True  # 选中了这个字母
                        self.current_letter = letter  # 则得到当前的这个字母
                        break  # 只要选中了就不循环了
            else:  # 如果底下已经排满了，想要修改的话，
                for letter, Rect in self.letter_Rect.items():
                    if Rect.y >= 215:  # 只能选择下面的字母
                        # 鼠标点击事件是点了以后一直响应
                        if Rect.collidepoint(self.word_maker.mouse_current_x, self.word_maker.mouse_current_y) \
                                and self.word_maker.click_event:
                            game_Sound('game_sound/mouse_click_1.mp3', 0.2)  # 游戏鼠标的声音
        ...
        # 第二步 已经选中了一个字母，这时判断这个字母是在哪
        if self.letter_contact:
            # 如果字母在底下的框中  等于-1代表不在底下的框中
            if self.letter_Rect[self.current_letter].collidelist(self.Blocks_Rect[self.current_attempt]) != -1:
            ...
            else:  # 不在底下的框中就是外面的字母，则放入对应的首个框的位置
                # 找到空余框的第一个位置
                self.letter_coordinate[self.current_letter][0] = self.Blocks_Rect[self.current_attempt][
                                                                     self.unreserved[0]].x + 15
                self.letter_coordinate[self.current_letter][1] = self.Blocks_Rect[self.current_attempt][
                                                                     self.unreserved[0]].y + 15
            self.letter_contact = False  # 字母进入对应的框里面以后鼠标就不会碰撞了
        ...
        # 去除已经占用的框
        self.unreserved = [val for val in self.contact_rect_list if val not in self.occupied_rect]  # b中有而a中没有的
    ...
    def check_Spelling(self):
            ...
            for i in range(len(self.player_used_spelling)):
                ...
                if self.player_used_spelling[i] == self.current_word:
                    ...
                    if self.gameplay_time > self.start_check_time + 2000:  # 如果回答正确展示2秒
                        ...
                        self.save_player_feature.append(self.click_pronunciation)  # 玩家主动听发音的次数(点击方式)
                        self.press_Enter = self.press_Enter - self.click_Enter  # 重复统计了，要减去按enter的次数
                        self.save_player_feature.append(self.press_Enter)  # 玩家被动听发音的次数
                        self.save_player_feature.append(self.click_Enter)  # 玩家主动听发音的次数 （点击方式）
                        for i in range(len(self.player_used_spelling)):
                        ...
                        self.word_maker.finished_tasks[self.current_word] = \
                            [self.tasks_parameters_list[self.task_index][1],
                             self.tasks_parameters_list[self.task_index][2],
                        ...
                        self.cumulative_time = True  # 累积时长
        ...
        if self.task_index == len(self.tasks_parameters_list):
            ...
            if self.countdown > 0:
                self.word_maker.saved_time = self.countdown  # 将积累的时长赋值
            else:
                self.word_maker.saved_time = 0
        ...
        if self.current_word not in self.player_used_spelling and \
            ...
            if self.gameplay_time > self.start_check_time + 4000:
                ...
                self.player_spelling = []  # 将这个记录清空
                self.task_change = True  # 修改参数
                self.cumulative_time = True  # 开始累计时长
                self.task_index += 1  # 并且进行到下一个任务
        # 如果已经到了最后一个单词
        if self.task_index == len(self.tasks_parameters_list):
            write_player_features_xls(self.save_player_features)  # 将玩家的数据写入文件
    ...
    def draw_word_parameters(self):
        ...
        # 右边中间第二条线
        pygame.draw.line(self.game_level_surface, (0, 0, 0), (720, 380), (self.surface_width, 380), 2)
    ...
    def draw_progress_bar(self, time):
        ...
        pygame.draw.rect(self.game_level_surface, (0, 0, 0), ((0, self.surface_height - 60), (self.surface_width, 50)),
                         width=4)  # 首先画一个边框
        # 这个是为了控制每一个单词的时间不一样
        if self.cumulative_time:
            self.total_time = time + self.countdown  # 任务时间=基础时长+剩余时长
            self.countdown = time + self.countdown  # 任务时间=基础时长+剩余时长
            self.decrease_width = self.surface_width / self.total_time   # 1秒减少多少宽度
            self.time_change = False  # 将开关关闭
            self.cumulative_time = False  # 不再累积时间
        # 检查完毕任务完成，则时间要暂停
        if not self.time_pause:
            self.task_second = self.total_time - self.countdown  # 记录学生玩这一关用了多少秒
        ...
        # 展示任务时长
        self.draw_Left_Text("Game_Fonts/chinese_pixel_font.TTF", '任务时长:' + str(self.total_time) + '秒', 40, 720, 0)
        # 当前任务难度
        self.draw_Left_Text("Game_Fonts/chinese_pixel_font.TTF",
                            '任务难度:' + str(self.tasks_parameters_list[self.task_index][5]) + '级'
                            , 40, 720, 50)
        # 展示当前得分
        self.draw_Left_Text("Game_Fonts/chinese_pixel_font.TTF", '当前得分:' + str(self.word_maker.player_score), 40, 720,
                            100)
        # 首先计算还剩多少个单词
        ...
        # 统计已经记住的单词
        self.draw_Left_Text("Game_Fonts/chinese_pixel_font.TTF", '记住单词:' + str(self.word_maker.remembered_words_number),
                            40, 720, 200)
        # 点击‘点击发音’也可以发音
        # 创建一个返回的按钮功能，放到左下角的位置
        click_pronunciation_button = self.draw_Menu_Text("Game_Fonts/chinese_pixel_font.TTF", '点此发音', 40, 860,
                                                         270, (0, 100, 0))
        if click_pronunciation_button.collidepoint(self.word_maker.mouse_current_x, self.word_maker.mouse_current_y):
            self.draw_Menu_Text("Game_Fonts/chinese_pixel_font.TTF", '点此发音', 40, 860,
                                270, (0, 255, 0))  # 创建一个返回的按钮功能，放到左下角的位置
        if click_pronunciation_button.collidepoint(self.word_maker.mouse_click_x, self.word_maker.mouse_click_y) and \
            ...
            # 创建一个变量统计点击的次数
            self.click_pronunciation += 1  # 玩家采用点击的方式听听力的次数
            if self.click_pronunciation == 3:
                self.word_maker.player_score += 1  # 玩家的分数加1
            game_Sound('UK_Pronunciation/' + self.current_word + '.mp3')

        # 检查拼写按钮
        check_spelling_button = self.draw_Menu_Text("Game_Fonts/chinese_pixel_font.TTF", '点此检查拼写', 40, 860,
                                                         315, (0, 100, 0))
        if check_spelling_button.collidepoint(self.word_maker.mouse_current_x, self.word_maker.mouse_current_y):
            self.draw_Menu_Text("Game_Fonts/chinese_pixel_font.TTF", '点此检查拼写', 40, 860,
                                315, (0, 255, 0))  # 创建一个返回的按钮功能，放到左下角的位置
        # 调用check_spelling函数 并且把enter键设置为真
        if check_spelling_button.collidepoint(self.word_maker.mouse_click_x, self.word_maker.mouse_click_y) and \
                self.word_maker.present_word_menu_chance:
            self.click_Enter += 1  # 玩家采用点击的方式检查拼写的次数
            # 先把回车键设置为真
            self.word_maker.check_spelling = True
            self.check_Spelling()
            self.word_maker.check_spelling = False
        # # 跳过单词按钮
        # check_spelling_button = self.draw_Menu_Text("Game_Fonts/chinese_pixel_font.TTF", '点此跳过单词', 40, 860, 364, (0, 100, 0))
        # if check_spelling_button.collidepoint(self.word_maker.mouse_current_x, self.word_maker.mouse_current_y):
        #     self.draw_Menu_Text("Game_Fonts/chinese_pixel_font.TTF", '点此跳过单词', 40, 860, 364, (0, 255, 0))
        #     # 如果点击了跳过单词按钮，并且当前难度不是1和4,只要打过题目就不能跳，以及时间耗尽，则跳到下一个单词
        # if check_spelling_button.collidepoint(self.word_maker.mouse_click_x, self.word_maker.mouse_click_y) and \
        #         self.word_maker.present_word_menu_chance and self.tasks_parameters_list[self.task_index][5] not in [1,4] and \
        #     self.current_attempt == 0 and self.countdown >= 0:
        #     self.task_change = True  # 修改参数
        #     self.task_index += 1  # 并且进行到下一个任务
        ...
        self.draw_Left_Text("Game_Fonts/chinese_pixel_font.TTF", '点击上方字母拼写', 30, 720, 408)
        self.draw_Left_Text("Game_Fonts/chinese_pixel_font.TTF", '点击框中字母取消', 30, 720, 440)
        # 操作指引
        self.draw_Left_Text("Game_Fonts/chinese_pixel_font.TTF", '方框的行数代表机会', 30, 720, 480)
        # 操作指引
        self.draw_Left_Text("Game_Fonts/chinese_pixel_font.TTF", '红色:单词没有该字母', 30, 720, 520)
        # 操作指引
        self.draw_Left_Text("Game_Fonts/chinese_pixel_font.TTF", '黄色:单词中有该字母', 30, 720, 560)
        self.draw_Left_Text("Game_Fonts/chinese_pixel_font.TTF", '但位置不正确', 30, 720, 590)
        # 操作指引
        self.draw_Left_Text("Game_Fonts/chinese_pixel_font.TTF", '绿色:字母完全正确', 30, 720, 630)
        self.draw_Left_Text("Game_Fonts/chinese_pixel_font.TTF", '按Q发音', 30, 720, 664, (0, 100, 0))
        self.draw_Left_Text("Game_Fonts/chinese_pixel_font.TTF", '按回车检查拼写', 30, 720, 697, (0, 100, 0))
        # 控制单词展示的时间
        if datetime.now() > self.start_time + timedelta(seconds=1):  # 如果时间间隔相差一秒
            self.start_time = datetime.now()  # 将现在的时间给过去的时间
            self.countdown -= 1
            if self.task_second in np.arange(5, time, 10) and not self.time_pause:  # 每过10秒，单词自动发音
                game_Sound('UK_Pronunciation/' + self.tasks_parameters_list[self.task_index][0] + '.mp3')
        ...
        if self.countdown < 0:
            self.show_Failure()
            # 展示4秒以后，进入下一个任务
            if self.gameplay_time > self.lock_time + 4000 and self.time_pause:
                self.save_player_feature.append(self.task_second)  # 添加玩家用了多少时间
                self.save_player_feature.append(self.current_attempt)  # 添加玩家用了多少次机会
                self.save_player_feature.append(0)  # 0代表玩家错了
                self.save_player_feature.append(self.press_Q)  # 玩家主动听发音的次数
                ...
                # 如果拼写为空# {单词：[音标，翻译，未完成，任务难度，文件label]}
                    ...
                    self.word_maker.finished_tasks[self.current_word] = \
                        [self.tasks_parameters_list[self.task_index][1], self.tasks_parameters_list[self.task_index][2],
                         '未完成', self.tasks_parameters_list[self.task_index][5],
                         self.tasks_parameters_list[self.task_index][3]]
                    # {单词：[音标，翻译，玩家拼写]} 这个是为了让玩家学习全部单词
                    self.word_maker.all_tasks[self.current_word] = \
                        [self.tasks_parameters_list[self.task_index][1], self.tasks_parameters_list[self.task_index][2],
                ...
                self.cumulative_time = True  # 开始累计时间
                self.task_index += 1  # 并且进行到下一个任务
                ...
                if self.task_index == len(self.tasks_parameters_list):
                    write_player_features_xls(self.save_player_features)  # 将玩家的数据写入文件
                    # 如果是耗尽时间完成任务的这种情况下
                    self.word_maker.saved_time = 0  # 没有积累任何时间，所以要将积累的时间清零
                    self.task_index = 0  # 让任务是第一个
                    self.word_maker.feedback_train_menu = GameFeedback(self.word_maker)  # 实例化反馈菜单
    ...
    def show_Finished_Task(self):
        ...
        for key, items in self.word_maker.finished_tasks.items():
            ...
            for index in range(len(items)):
                if index == 1:  # 汉语
                    self.draw_Left_Text("Game_Fonts/chinese_pixel_font.TTF", items[index], 40, x_coordinate,
                                        y_coordinate)
                else:
                    pass
            if x_coordinate < 360:
    ...
    def display_menu(self):
        ...
        if self.task_change:  # 如果时间改变，代表单词改变，所以要重新读取单词
            ...
            if self.tasks_parameters_list[self.task_index][0] in self.word_maker.word_loop:  # 如果已经有这个单词
                self.word_maker.word_loop[self.tasks_parameters_list[self.task_index][0]] += str(
                    self.tasks_parameters_list[self.task_index][5])
            else:
                self.word_maker.word_loop[self.tasks_parameters_list[self.task_index][0]] = str(
                    self.tasks_parameters_list[self.task_index][5])
            ...
            self.click_pronunciation = 0  # 玩家采用点击的方式主动听发音的次数归0
            self.click_Enter = 0  # 玩家采用点击进行拼写检查的次数归0
            self.yellow_color = 0  # 统计总共的黄色的次数
            self.green_color = 0  # 统计总共的绿色的个数
            self.red_color = 0  # 统计总共的红色的个数
            self.red_color_dic = {}  # 统计哪个字母迷惑了玩家
            ...
            if self.countdown < 0:  # 切换任务的时候，如果倒计时小于0，要将其修改为0
                self.countdown = 0
        ...
        # 如果最后一次机会已经用完移动字母就不在起作用,并且时间用完了也不能移动字母了
        if self.current_attempt != self.tasks_parameters_list[self.task_index][6] and self.countdown >= 0:
