...
class GameFeedback(object):
    ...
    def __init__(self, word_maker):
        ...
        self.countdown = 60 + int(self.word_maker.saved_time/2)  # 初始化倒计时，用来记录已经过了多久
        self.feedback_time = 60 + int(self.word_maker.saved_time/2)  # 反馈展示30秒
        self.word_maker.saved_time = int(self.word_maker.saved_time / 2)  # 将节省的时间减少一半重新分配到任务
    ...
    def finished_Tasks(self):
        ...
        # 先展示一句话
        self.draw_Left_Text("Game_Fonts/chinese_pixel_font.TTF", '你有'+str(self.feedback_time)+'秒时间查看反馈', 40, 300, 0)
        # 在展示每一列的列标题
        ...
        for key, items in self.all_tasks.items():
            if key == items[2]:  # 如果玩家拼写正确
                ...
                self.draw_Left_Text("Game_Fonts/chinese_pixel_font.TTF", key, 40, x_coordinate, y_coordinate,COLOR=(200, 0, 0))
                for index in range(len(items)):
                    if index == 0:  # 音标
                        # 音标
                        self.draw_Left_Text("Game_Fonts/phonetic.ttf", items[index], 30,
                                            x_coordinate + (index + 1) * self.x_increase, y_coordinate,COLOR=(200, 0, 0))
                    elif index == 1:  # 汉语
                        self.draw_Left_Text("Game_Fonts/chinese_pixel_font.TTF", items[index], 40, \
                                            x_coordinate + (index + 1) * self.x_increase, y_coordinate,COLOR=(200, 0, 0))
                    else:  # 玩家拼写错误的单词得重点标注
        ...
        if datetime.now() > self.start_time + timedelta(seconds=1):  # 如果时间间隔相差一秒
            ...
            if self.countdown == -1:  # 如果时间结束，则进入游戏界面
                ...
                if self.word_maker.current_loop == 5:  # 设置最大的游戏轮数 （1，2，3，4）
                    self.player_end_game = (time.perf_counter() - self.word_maker.start_play_time)
