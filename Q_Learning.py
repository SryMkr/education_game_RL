...
N_STATES = 5   # 状态集合
ACTIONS = ["right", "left"]  # 左右移动分别代表对错
EPSILON = [0.6, 0.7, 0.8, 0.9]  # 又10%的概率随机选择动作，有90%的概率选择收益最大的动作
learning_rate = 0.1  # 学习率 要是加的多了 超过后面的值就不合适了
GAMMA = 0.9  # 折扣奖励
MAX_EPISODES = 7  # 玩多少次游戏，拿到奖励算一次
...
def build_q_table(n_states, actions):
    return pd.DataFrame(
        np.zeros((n_states, len(actions)), dtype=int),  # 初始化Q_TABLE
        columns=actions  # 列名
    )
